...
class cur2IKFX_Tool():
    ...
    def __init__(self):
        self.pyside = cur2IKFX_ToolUi()
        self.pyside.RebuildIntText.setText(u"重建段数")
        self.pyside.RebuildInt.setPlaceholderText(u"重建段数")
        self.pyside.CurveNameText.setText(u"曲线名")
        self.pyside.CurveName.setPlaceholderText(u"曲线名")
        self.pyside.SelectPolyCurve.setText(u"选模型的线")
        self.pyside.SelectPolyCurve.clicked.connect(lambda *args: self.SelectPolyCurve())
        self.pyside.reverseCurve.setText(u"反转曲线")
        self.pyside.reverseCurve.clicked.connect(lambda *args: self.reverseCurve())
        self.pyside.FromJointBG.setText(u"从骨骼开始")
        self.pyside.FromJointBG.clicked.connect(lambda: self.setdisable())
        self.pyside.FromJointWar.setText(u"选择开始骨骼和结束骨骼\n不能进行批量创建")
        self.pyside.SkinCtrlbox.setText(u"骨骼控制-数量")
        self.pyside.FXCurvebox.setText(u"添加动力学")
        self.pyside.JointIntText.setText(u"骨骼段数")
        self.pyside.HairSystemText.setText(u"HairSystem")
        self.pyside.BuildCtrl.setText(u"创建")
        self.pyside.BuildCtrl.clicked.connect(lambda *args: self.createCtrl())
        self.pyside.PoseEdit.setText(u"Adv默认Pose编辑")
        self.pyside.PoseEdit.clicked.connect(lambda *args: self.PoseCheck())
        self.pyside.tabWidget.setTabText(self.pyside.tabWidget.indexOf(self.pyside.child1), u"本体")
        self.pyside.SelCtrlCurve.setText(u'选择控制器')
        self.pyside.SelCtrlCurve.clicked.connect(lambda *args: self.SelCurve())
        self.refreshNode()
        self.pyside.show()        
